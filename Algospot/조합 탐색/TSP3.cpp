...
double mintEdge[MAX];
int n;	//도시의 수
double dist[MAX][MAX];	//도 도시간의 거리를 저장하는 배열
//지금까지 찾은 최적해
double best;
...
//dp(here,visited)=cache[here][남은도시의 수][visited]
map<int, double> cache[MAX][CACHED_DEPTH + 1];

int countTrue(int bit) {	//비트로 visited가 표현되어 있음
	if (bit == 0)
		return 0;
