/*
아래 풀이가 처음 스도쿠를 접했을 때 이해가 간 풀이였다.
이렇게 말고 행, 열, 3*3사각형을 만들지 않고 순회하면서 확인하는 방법도 있다.
*/

import java.io.BufferedReader;
import java.io.InputStreamReader;
import java.util.StringTokenizer;

public class Main {
	
	final static int MAX = 9;
	static int sudoku[][] = new int[MAX][MAX];
	// row는 행, col은 열, square는 3*3 사각형.
	// row[i][j] = i번째 행,열,사각형에 j라는 숫자가 있는지 없는지 여부
	static boolean row[][] = new boolean[MAX+1][MAX+1];
	static boolean col[][] = new boolean[MAX+1][MAX+1];
	static boolean square[][] = new boolean[MAX+1][MAX+1];
	
	static void solve(int num) {
		if(num == 81) {
			StringBuilder sb = new StringBuilder();
			for(int i=0; i<9; ++i) {
				for(int j=0; j<9; ++j) {
					sb.append(sudoku[i][j]).append(' '); 
				}
				sb.append('\n'); 
			}
			System.out.println(sb);
			System.exit(0);
		}
		
		int y = num/9;
		int x = num%9;
		
		if(sudoku[y][x] == 0) {
			for(int i=1; i<=9; ++i) {
				// (y,x) 에 해당하는 행, 열, 사각형에 1~9 중 숫자가 없으면 삽입하고 진행
				if(!row[y][i] && !col[x][i] && !square[(y/3*3)+x/3][i]) {
					row[y][i] = true;
					col[x][i]  = true;
					square[(y/3*3)+x/3][i] = true;
					sudoku[y][x] = i;
					solve(num+1);
					row[y][i] = false;
					col[x][i]  = false;
					square[(y/3*3)+x/3][i] = false;
					sudoku[y][x] = 0;
				}
			}
		}
		else
			solve(num+1);
	
	}
	
	public static void main(String[] args) throws Exception {
		
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		StringTokenizer st;
		
		for(int i=0; i<9; ++i) {
			st = new StringTokenizer(br.readLine(), " ");
			for(int j=0; j<9; ++j) {
				sudoku[i][j] = Integer.parseInt(st.nextToken());
				if(sudoku[i][j]==0)
					continue;
				row[i][sudoku[i][j]] = true;
				col[j][sudoku[i][j]] = true;
				square[(i/3*3)+j/3][sudoku[i][j]] = true;
			}
		}
		
		solve(0);
	}
}



// 배열 순회하면서 알아내는 방법
import java.io.BufferedReader;
import java.io.InputStreamReader;
import java.io.IOException;
import java.util.StringTokenizer;
 
public class Main {
 
	public static int[][] arr = new int[9][9];
 
	public static void main(String[] args) throws IOException {
 
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
 
		for (int i = 0; i < 9; i++) {
			StringTokenizer st = new StringTokenizer(br.readLine(), " ");
			for (int j = 0; j < 9; j++) {
				arr[i][j] = Integer.parseInt(st.nextToken());
			}
		}
 
		sudoku(0, 0);
 
	}
 
	public static void sudoku(int row, int col) {
 
		// 해당 행이 다 채워졌을 경우 다음 행의 첫 번째 열부터 시작
		if (col == 9) {
			sudoku(row + 1, 0);
			return;
		}
 
		// 행과 열이 모두 채워졌을 경우 출력 후 종료
		if (row == 9) {
			StringBuilder sb = new StringBuilder();
			for (int i = 0; i < 9; i++) {
				for (int j = 0; j < 9; j++) {
					sb.append(arr[i][j]).append(' ');
				}
				sb.append('\n');
			}
			System.out.println(sb);
			// 출력 뒤 시스템을 종료한다.
			System.exit(0);
		}
 
		// 만약 해당 위치가 0 이라면 1부터 9까지 중 가능한 수 탐색
		if (arr[row][col] == 0) {
			for (int i = 1; i <= 9; i++) {
				// i 값이 중복되지 않는지 검사
				if (possibility(row, col, i)) {
					arr[row][col] = i;
					sudoku(row, col + 1);
				}
			}
			arr[row][col] = 0;
			return;
		}
 
		sudoku(row, col + 1);
 
	}
 
	public static boolean possibility(int row, int col, int value) {
 
		// 같은 행에 있는 원소들 중 겹치는 열 원소가 있는지 검사
		for (int i = 0; i < 9; i++) {
			if (arr[row][i] == value) {
				return false;
			}
		}
 
		// 같은 열에 있는 원소들 중 겹치는 행 원소가 있는지 검사
		for (int i = 0; i < 9; i++) {
			if (arr[i][col] == value) {
				return false;
			}
		}
 
		// 3*3 칸에 중복되는 원소가 있는지 검사
		int set_row = (row / 3) * 3; // value가 속한 3x3의 행의 첫 위치
		int set_col = (col / 3) * 3; // value가 속한 3x3의 열의 첫 위치
 
		for (int i = set_row; i < set_row + 3; i++) {
			for (int j = set_col; j < set_col + 3; j++) {
				if (arr[i][j] == value) {
					return false;
				}
			}
		}
 
		return true; // 중복되는 것이 없을 경우 true 반환
	}
 
}